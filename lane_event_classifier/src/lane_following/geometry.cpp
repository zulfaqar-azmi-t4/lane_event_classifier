...
LaneFollowingGeometry::LaneFollowingGeometry(LaneFollowingConfig config) : config_{config}
{
}

std::optional<LaneFollowingReason> LaneFollowingGeometry::is_lane_following(
  const lanelet::LaneletMapPtr & map, const ReferenceLane & reference,
  const std::unordered_set<lanelet::Id> & sequence_ids, const lanelet::BasicPoint2d & point) const
{
  // inside_connected_sequence (docs/lane_following.md, "Inside the connected sequence").
  if (is_point_inside_any(map, sequence_ids, point)) {
    return LaneFollowingReason::inside_connected_sequence;
  }

  // within_lateral_tolerance (docs/lane_following.md, "Within lateral tolerance").
  if (is_point_within_tolerance_of_any(map, sequence_ids, point, config_.lateral_tolerance_m)) {
    return LaneFollowingReason::within_lateral_tolerance;
  }

  // road_shoulder_exempt (docs/lane_following.md, "Road-shoulder exemption").
  if (config_.enable_road_shoulder_exemption && is_point_in_road_shoulder(map, point)) {
    return LaneFollowingReason::road_shoulder_exempt;
  }

  // turn_lane_exempt (docs/lane_following.md, "Turn / intersection-lane exemption").
  if (config_.enable_turn_lane_exemption && is_in_turn_lane(map, reference, sequence_ids, point)) {
    return LaneFollowingReason::turn_lane_exempt;
  }

  // virtual_boundary_exempt (docs/lane_following.md, "Virtual-boundary exemption").
  if (
    config_.enable_virtual_boundary_exemption &&
    crossed_sequence_boundary_is_virtual(map, sequence_ids, reference, point)) {
    return LaneFollowingReason::virtual_boundary_exempt;
  }
